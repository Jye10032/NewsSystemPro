...
const DASHBOARD_CACHE_KEY = 'news-system-dashboard-home-cache-v1'
...
let dashboardMemoryCache: DashboardCacheRecord | null = null

function readDashboardCache(): DashboardHomeResponse | null {
    const now = Date.now()

    if (dashboardMemoryCache && dashboardMemoryCache.expiresAt > now) {
...
function writeDashboardCache(data: DashboardHomeResponse) {
    const record: DashboardCacheRecord = {
        expiresAt: Date.now() + DASHBOARD_CACHE_TTL,
        data
    }
    dashboardMemoryCache = record
    ...
    try {
        window.sessionStorage.setItem(DASHBOARD_CACHE_KEY, JSON.stringify(record))
    } catch {
...
export default function Home() {
    ...
    const [newsViewList, setNewsViewList] = useState<NewsItem[]>(cachedDashboard?.topViewed || [])
    const [newsStarList, setNewsStarList] = useState<NewsItem[]>(cachedDashboard?.topLiked || [])
    ...
    const [categoryDistribution, setCategoryDistribution] = useState<ChartDatum[]>(
        cachedDashboard?.categoryDistribution || []
    )
    const [currentUserSummary, setCurrentUserSummary] = useState(
        cachedDashboard?.currentUserSummary || {
            totalNews: 0,
            totalViews: 0,
            totalLikes: 0,
    ...
    const [summaryLoading, setSummaryLoading] = useState(!cachedDashboard)
