...
describe('TopHead 顶部栏组件', () => {
  ...
  describe('侧边栏伸缩功能', () => {
    ...
    it('应该能够连续切换侧边栏状态', async () => {
      ...
      const foldIcon = container.querySelector('.anticon-menu-fold')
      await user.click(foldIcon)
      expect(store.getState().collapsible).toBe(true)
      ...
      const unfoldIcon = container.querySelector('.anticon-menu-unfold')
      await user.click(unfoldIcon)
      expect(store.getState().collapsible).toBe(false)
    })
  })
